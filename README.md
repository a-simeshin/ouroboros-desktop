...
> - **🔱 [fork] Удалена интеграция с Claude Code (Claude Agent SDK)** — из runtime-агента полностью убраны инструмент делегированной правки `claude_code_edit`, advisory-инструмент пред-коммитного ревью `advisory_pre_review`, SDK-транспорт (`gateways/claude_code.py`), SDK-валидация рантайма (`claude_runtime.py`), машинерия advisory-прогонов (`AdvisoryRunRecord`/freshness/`skip_advisory_pre_review`), настройка `CLAUDE_CODE_MODEL` и эндпоинты `/api/claude-code/status|install`; runtime-зависимость `claude-agent-sdk` снята из `requirements.txt`/`pyproject.toml`. Мотивация — сокращение attack surface и headless/k8s-окружение; замены нет. **Блокирующая ревью-триада (`OUROBOROS_REVIEW_ENFORCEMENT`, триада + scope, obligations/debts) сохранена.** Спека: [`openspec/specs/ouroboros/ouroboros.md`](openspec/specs/ouroboros/ouroboros.md) (раздел 4.6).
...
- **Anthropic** — direct runtime routing (`anthropic::claude-opus-4.6`, etc.)
