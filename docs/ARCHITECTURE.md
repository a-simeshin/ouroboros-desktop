...
server.py (Starlette+uvicorn) ← single main process; HTTP + WebSocket on configurable host:port (default localhost:8765; Docker/non-loopback supported via OUROBOROS_SERVER_HOST=0.0.0.0)
  ...
  └── ouroboros/               ← Agent core (runs inside worker processes)
      ...
      ├── review_state.py      ← Durable blocking review-ledger state (commit-attempt records, obligations, debts, blocking history)
      ...
      ├── runtime_mode_policy.py ← Runtime-mode protected-path policy (safety-critical files, frozen contracts, release/managed invariants) shared by registry and git tools
      ...
      │   ├── commit_gate.py     ← Commit-attempt recording (extracted from git.py); `_record_commit_attempt` runs LLM-based claim synthesis (via `review_synthesis.py`) on blocked attempts before durable obligations are created
...
│   │   ├── advisory_review.json ← Durable blocking-review ledger (commit attempts, obligations, commit-readiness debts; historical filename)
...
- **Models tab**: Main, Code, Light, Fallback model routing. Each card has a `Local` toggle to route through the GGUF server configured in Advanced.
...
- All tools have hard timeout (default 600s, per-tool overrides for search); `OUROBOROS_TOOL_TIMEOUT_SEC` in `settings.json` is the runtime SSOT override read on each tool call. The actual timeout is `max(settings_value, per_tool_declared)` so tools declaring a higher minimum are never silently capped by a lower global default.
...
- `stage_adaptations` stages Ouroboros adaptation changes without committing. Call it immediately before `stage_pr_merge` — do NOT run `repo_commit` in between, because `repo_commit` always checks out `branch_dev` (ouroboros) first, which drops off the integration branch and discards the staged state. Staged adaptation changes survive into the final merge commit created by `repo_commit` after `stage_pr_merge`.
...
7. `stage_pr_merge(branch="integrate/pr-N")` → `repo_commit` — finalise through the standard reviewed pipeline; creates a merge commit with both parents (staged adaptations from step 6 land here)
...
- **Edit workflow**: choose the right edit tool first — `str_replace_editor` for one
  exact replacement, `repo_write` for new files or intentional full rewrites — then
  finish all edits and call `repo_commit`.
...
- **`repo_write_commit`**: legacy single-file write+commit (kept for compatibility).
  Runs unified review before commit.
...
and the scope reviewer prompt (`scope_review.py`). A parallel condensed "Critical threshold rule"
...
`_resolve_matching_obligations` resolves by both `obligation_id` (primary) and
`item.lower()` (fallback for legacy saved state), so the dual-path resolution handles
obligations created under either scheme.
...
expensive LLM review. Returns a list of warning strings (empty = ready):
1. **No uncommitted changes** — blocking: if git status shows nothing to review, the
   commit gate returns an error immediately.
...
4. **Diff size warning** — warns if combined staged+unstaged diff exceeds review thresholds.

Non-blocking warnings are surfaced to the agent rather than hard-blocking the commit.
...
The staged-index equivalent in `_preflight_check` (review.py) remains separate — it reads
from `git show :PATH` after staging and is the authoritative gate.
...
- Reflection prompt now includes structured review evidence (recent reviewed attempts,
  open obligations, commit-readiness debt, continuations) so blocked-review lessons
...
- Task results also persist `review_evidence` so blocked-review history, obligations, and
  commit-readiness debt can feed summaries, reflections, and later diagnostics from the same durable record.
...
### Review stack (triad + scope in parallel → commit)

The commit pipeline runs the blocking review stage before creating a git commit:

1. **Triad diff review** (`tools/review.py`) + **Blocking scope review** (`tools/scope_review.py`) — run **concurrently** via `ThreadPoolExecutor` (orchestrated in `tools/parallel_review.py`). Both execute on the same staged snapshot. The agent receives all findings (triad + scope) in a single blocking round.
...
#### Blocking review ledger

- **Pre-review test preflight**: before the parallel triad + scope review, `_run_reviewed_stage_cycle` in `ouroboros/tools/git.py` calls `review_helpers._run_review_preflight_tests(ctx)` immediately before `_fingerprint_staged_diff` / the parallel review runs. It runs `[agent_python, "-m", "pytest", "tests/", "-q", "--tb=line", "--no-header"]` (timeout 120s) in the worktree; `agent_python` resolves as `sys.executable or OUROBOROS_AGENT_PYTHON or "python3"` so packaged builds use the bundled interpreter instead of PATH. Two explicit skip paths remain: `skip_tests=True`, and diff-aware doc-only bypass for prose `.md`/`.txt`/`.rst` files outside any `tests/` path. JSON, config, schema, package metadata, and code-to-doc renames are not doc-only and still run preflight. A test failure here unstages the diff (`git reset HEAD`), records a `_record_commit_attempt` with `block_reason="tests_preflight_blocked"`, and returns a blocked result — the LLM review is never invoked. Respects the `OUROBOROS_PRE_PUSH_TESTS=0` env gate (same as the post-commit test runner).
- **Obligation-based blocking history injection** (v4.12.0, expanded in v4.40.4): when previous
  ...
  round-trips the prior `obligation_id`. When open obligations exist, the blocking triad + scope
  prompt includes an "Unresolved obligations from previous blocking rounds" section listing each
  obligation by id, item, severity, and reason. Reviewers MUST explicitly address each obligation
  by checklist item name. Blocking history and open obligations are repo-scoped. The prompt
...
- **Commit-readiness debt layer** (v4.40.4): recurring blocked-review signals are also mirrored into
  ...
  obligations and readiness warnings. `agent_task_pipeline.build_review_context()`
...
- **Obligation resolution**: `_resolve_matching_obligations()` runs on every parseable review
  result (regardless of whether other items fail). An obligation is marked resolved only when the
  reviewer emits an unambiguous PASS for its checklist item (PASS present AND no FAIL for the same
  item in the same run). `on_successful_commit()` clears all obligations on a successful commit.
- **`review_status`** tool: read-only diagnostic showing open obligations,
  open commit-readiness debt, last commit attempt state, and a concrete
  next-step recommendation. Returns structured JSON with: `open_obligations_count`,
...
- **`review_state.py`**: durable state. State file: `data/state/advisory_review.json`.
  Stores a typed reviewed-attempt ledger, bounded blocking-attempt history,
  open obligations, repo/tool/task identities, and reviewed-diff fingerprints.
  Commit attempts carry **forensic fields** `triad_models` (list of configured
  ...
  `tests_preflight_blocked` means pytest failed before the LLM review (v4.41.0+) — the
  agent must fix failing tests and re-run `repo_commit` (or pass skip_tests=True).
  Commit attempts have: `status` (reviewing/blocked/succeeded/failed), `block_reason`
  (critical_findings/review_quorum/parse_failure/infra_failure/scope_blocked/preflight/overlap_guard/revalidation_failed/fingerprint_unavailable),
  ...
  `next_obligation_seq`, and `next_commit_readiness_debt_seq`, plus explicit lock-backed state updates.
  **Epistemic-integrity fields** (v4.32.0): `triad_raw_results` (list of per-model actor dicts:
  ...
  permanently unready. The commit gate treats open commit-readiness debt as a blocking signal,
  so the commit gate and `review_status` agree.
...
- **`review_evidence.py`**: structured collector that snapshots review ledger state,
  open obligations, open commit-readiness debt, and continuations into `task_results`, task summaries, and
  execution reflections. When `repo_dir` / `repo_key` is known, open obligations stay
  ...
  Output dict fields: `recent_attempts` (scoped commit attempts),
  `open_obligations`, `commit_readiness_debts`, `continuations`, `corrupt_continuations`,
  plus **omission counters** `omitted_attempts`, `omitted_obligations`,
  ...
  **forensic fields** from attempt records (via `_attempt_to_dict`): `triad_models`, `scope_model`,
  ...
  `has_evidence` is `True` when any visible list is non-empty OR any
  omission counter is > 0 — including `omitted_corrupt` — so truncated evidence is never silently
  treated as absent. `max_attempts=0` correctly returns an empty visible list with
  all entries counted as omitted (Python `[-0:]` full-slice bug is guarded explicitly).
- **Snapshot fingerprint**: deterministic SHA-256 of the staged-diff content; the reviewed-diff
  fingerprint is recorded on each commit attempt for pre/post overlap-guard checks.
- **Gate integration**: both `_repo_commit_push` and `_repo_write_commit` record commit attempts
...
- **Shared reviewer calibration**: triad and scope prompts both inject the same
  ...
  critical) stays consistent across both review surfaces.
...
| ANTHROPIC_API_KEY | "" | Optional. Enables direct Anthropic runtime routing (`anthropic::...` model values) |
...
All subprocesses spawned by agent tools (`run_shell`)
...
- Repo review (triad + scope) protects the self-modifying
